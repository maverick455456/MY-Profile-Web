...
export default function CuteRobot() {
  ...
  const sayTyped = (text: string, speed = 22) => {
    typeText(text, speed);
    speakCute(text);
  };
  ...
  const setPupil = (id: 'pL' | 'pR', cx: number, cy: number, lookX: number, lookY: number) => {
    ...
    const p = wrap.querySelector<SVGCircleElement>(`#${id}`);
    if (!p) return;
  ...
  const onRobotClick = () => {
    ...
    pickWaypoint();
    sayTyped('Hello, I’m N-Tech Robot!', 22);
  };
  ...
  const onDockToggle = () => {
    ...
    if (docked.current) {
      target.current.x = 12;
      target.current.y = Math.max(100, window.innerHeight - 200);
      sayTyped('Docked. Click me to undock!', 18);
    } else {
      pickWaypoint();
      sayTyped('Undocked. Wandering around!', 20);
    }
  };
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        .shadow {
          position: absolute; left: 50%; bottom: 2px; width: 88px; height: 18px; transform: translateX(-50%);
          background: radial-gradient(50% 50% at 50% 50%, rgba(0,0,0,.36), rgba(0,0,0,0));
          filter: blur(2px); opacity: .56; transition: width .2s ease, opacity .2s ease;
        }
        .bubble {
          position: absolute; left: 100%; bottom: 70%; margin-left: 10px; max-width: 260px;
          font-size: 14px; line-height: 1.4; color: #fff;
          background: rgba(255,255,255,.10); border: 1px solid rgba(255,255,255,.18);
          border-radius: 12px; padding: 10px 12px; backdrop-filter: blur(10px);
          box-shadow: 0 10px 24px rgba(0,0,0,.28); opacity: 0; transform: translateY(6px);
          pointer-events: none;
        }
        .bubble.show { opacity: 1; transform: translateY(0); transition: opacity .2s ease, transform .2s ease; }
        .bubble :global(.cursor) {
          display: inline-block; width: 8px; margin-left: 2px; background: #ffffffcb;
          height: 14px; vertical-align: -2px; animation: blinkCursor 1s steps(1) infinite;
        }
        ...
        .dock-btn {
          position: absolute; right: -6px; top: -6px; width: 26px; height: 26px; border-radius: 999px;
        ...
        @media (prefers-reduced-motion: reduce) {
          .idle :global(.body), .idle :global(.prop), .idle :global(.lid) { animation: none; }
        }
      `}</style>
    </>
  );
